...
export function VariantModelScaleFlow({ variant, onConfirmed }) {
  ...
  const [axisMap, setAxisMap] = useState({ width: '', height: '', depth: '' })
  ...
  return (
    <section className="flex flex-col gap-4 border-t border-border pt-4" aria-label="Kích thước thật của mô hình 3D">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">Tải GLB lên R2, xác định hướng trục rồi nhập một số đo thật bằng centimet.</p>
      </div>
            ...
            {DIMENSIONS.map(([dimension, label]) => (
              <label key={dimension} className="flex flex-col gap-1 text-sm font-medium text-foreground">
                ...
                <select
                  ...
                  onChange={(event) => {
                    setAxisMap((current) => ({ ...current, [dimension]: event.target.value }))
                ...
                >
                  ...
                  {AXES.map((axis) => (
                    <option key={axis} value={axis} disabled={Object.entries(axisMap).some(([key, value]) => key !== dimension && value === axis)}>
