  ...
  return (
    <group>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0]} receiveShadow>
        ...
        <meshStandardMaterial color="#F7F7F3" /> {/* canvas */}
      </mesh>
      {/* Lưới KHÍT đúng hình phòng rộng×sâu, mỗi ô = 1m (giúp đọc tỉ lệ). `unbuilt`
          #D8D8CE. Trước đây gridHelper vuông max(w,d) → tràn ra ngoài tường. */}
      <Grid
        args={[width, depth]}
        position={[0, 0.01, 0]}
        cellSize={1}
      ...
      {v.back && (
        <mesh position={[0, height / 2, -depth / 2]}>
          ...
          <meshStandardMaterial color="#F7F7F3" transparent opacity={0.18} /> {/* canvas */}
        </mesh>
      )}
      {v.left && (
        <mesh position={[-width / 2, height / 2, 0]} rotation={[0, Math.PI / 2, 0]}>
          <planeGeometry args={[depth, height]} />
          <meshStandardMaterial color="#F7F7F3" transparent opacity={0.12} /> {/* canvas */}
        </mesh>
      )}
      {v.right && (
        <mesh position={[width / 2, height / 2, 0]} rotation={[0, -Math.PI / 2, 0]}>
