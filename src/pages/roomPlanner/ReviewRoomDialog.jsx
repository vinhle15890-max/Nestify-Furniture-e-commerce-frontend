...
const reasonCopy = {
  out_of_stock: 'Tạm hết hàng — hãy xóa món này để tiếp tục.',
  discontinued: 'Sản phẩm này không còn được bán.',
}

export function ReviewRoomDialog({ open, onOpenChange, items, review, loading, error, onContinue, pending, onRemove, removingPlacementId, removeError }) {
  ...
  return (
    <BecomingModal open={open} onOpenChange={onOpenChange} title="Xem lại phòng" description="Kiểm tra các món bạn đã đặt trước khi chuyển sang giỏ hàng.">
      ...
      {review?.items && <ul className="mt-5 divide-y divide-border border-y border-border">{review.items.map((item) => {
        ...
        return <li key={item.placement_id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-4 py-3"><div className="min-w-0"><p className="text-sm font-medium text-foreground">{item.product_name}</p><p className="text-xs text-muted-foreground">{item.variant_name ?? 'Phiên bản không còn khả dụng'} · {formatStock(item.available_stock)}</p>{!item.purchasable && <><p className="mt-1 text-xs text-destructive">{reasonCopy[item.reason] ?? 'Món này hiện chưa thể mua.'}</p><Button type="button" variant="ghost" className="mt-2 min-h-11 px-0 text-destructive hover:bg-transparent" aria-label={`Xóa ${item.product_name} khỏi phòng`} disabled={removing || pending} onClick={() => onRemove?.(item.placement_id)}>{removing ? <Spinner label={`Đang xóa ${item.product_name}`} /> : 'Xóa khỏi phòng'}</Button></>}</div><span className="text-sm tabular-nums text-foreground">{item.price == null ? '—' : formatPrice(item.price)}</span></li>
      })}</ul>}
