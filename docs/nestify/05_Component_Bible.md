...
*Kế thừa từ Design DNA (semantic màu/type/motif) và 04_Visual_Grammar.md
(composition, hierarchy, line, depth, light, illustration, rhythm, review).
Design DNA định nghĩa identity; Visual Grammar định nghĩa visual execution; tài
liệu này định nghĩa hành vi component trong từng trạng thái tâm lý. Tổ chức theo
Psychological State, không theo tên component — xem lý do ở Design DNA mục 0
(tránh overfit vào cấu trúc hiện tại).*
...
Button ở state này giữ secondary emphasis — không có CTA nào mang màu `imagined` hay `confirmed` (Design DNA mục 2) ở đây, vì người dùng chưa ở trạng thái Mentally Real hay Committed. Dùng `unbuilt` hoặc `ink` với outline nhẹ, theo grammar về evidence/materiality ở 04_Visual_Grammar.md §8; không biến outline thành technical wireframe.

**Visual Spec:** radius chỉ lặp lại khi vai trò interaction chung rõ ràng; không
dùng shadow đậm (shadow đậm ngụ ý "đã thành hình", sai với state Not Yet Seen).
Border `unbuilt` chỉ xuất hiện khi nó diễn đạt grouping hoặc state thật; width,
contrast, và edge role tuân theo 04_Visual_Grammar.md §5, không áp một 1px mặc
định cho mọi card. Không fill nền đậm.
...
Planner entry: một affordance dẫn vào Room Planner thật có thể giải thích ngắn
gọn giá trị, context sản phẩm được mang theo, hoặc khả năng quay lại mà không
mất ngữ cảnh. Nó không được là canvas thu nhỏ, không đặt/đo/snap/materialize
một object, và không mô phỏng Planner trước khi người dùng vào state sở hữu nó.
Chi phí quyết định "có nên mở Planner không" được giảm bằng một chuyển tiếp rõ
ràng và có thể quay lại, không bằng một capability giả (Design DNA mục 0–1).
...
**Visual Spec:** màu `emerging` (Design DNA mục 2) xuất hiện ở đây làm accent cho trạng thái "đang khám phá" — ví dụ progress indicator nhỏ khi model 3D đang tải, hoặc border tô sáng nhẹ khi card được hover. Placeholder model là evidence tạm thời về scale/vị trí, không phải lỗi cần che giấu; áp dụng grammar về depth và illustration ở 04_Visual_Grammar.md §6 và §8, không dùng fake technical sketch như trang trí.
...
Feedback: object "materialize" tức thì tại điểm thả — material truth xuất hiện sau khi spatial evidence đã rõ, theo 04_Visual_Grammar.md §6–8; không cần animation trung gian kéo dài.
