...
- Bộ lọc payment tách `PayOS chờ thanh toán` (chỉ order còn khả năng thanh toán) khỏi `Đơn cũ cần đối soát` (cancelled + legacy pending). Danh sách không gọi order đã hủy là đang chờ khách trả tiền. `confirmation_queue` là shortcut ẩn từ dashboard: khi URL có order/payment filter rõ ràng, FE bỏ shortcut này khỏi request và canonical URL để tránh giao rỗng như `awaiting_online_payment + delivered|closed`. Sau khi backend mới được deploy, `delivered + COD pending` dùng endpoint thu COD tương thích legacy và không gửi transition `delivered -> delivered`.
